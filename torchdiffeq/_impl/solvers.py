...
class FixedGridODESolver(metaclass=abc.ABCMeta):
    ...
    def integrate(self, t):
        time_grid = self.grid_constructor(self.func, self.y0, t) ### time T Todo
...
class FixRandomisedGridODESolver(metaclass=abc.ABCMeta):
    order: int
    ...
    def integrate(self, t):
        ...
        generator_tau = torch.Generator(device=self.y0.device)

        generator_tau.manual_seed(19960408)
        tau = torch.randn(t.size(0), dtype=self.y0.dtype,device=self.y0.device, generator=generator_tau)
        assert time_grid[0] == t[0] and time_grid[-1] == t[-1]
        solution = torch.empty(len(t), *self.y0.shape, dtype=self.y0.dtype, device=self.y0.device)
        solution[0] = self.y0
        j = 1
        ...
        for t0, t1 in zip(time_grid[:-1], time_grid[1:]):
            ...
            tau0 = tau[j-1]
            self.func.callback_step(t0, y0, dt)
            dy, f0 = self._step_func(self.func, t0, tau0, dt, t1, y0)
            y1 = y0 + dy
...
class RandomRandomisedGridODESolver(metaclass=abc.ABCMeta):
    order: int
    ...
    def integrate(self, t):
        time_grid = self.grid_constructor(self.func, self.y0, t) ### time T Todo
        # import numpy as np
        # np.random.seed(19960408) #Todo
        # tau = np.random.randn(t.size(0))
        # tau = torch.from_numpy(tau).to(device=self.y0.device)
        # torch.manual_seed(19960408)
        tau = torch.randn(t.size(0), dtype=self.y0.dtype,device=self.y0.device)
        assert time_grid[0] == t[0] and time_grid[-1] == t[-1]
        solution = torch.empty(len(t), *self.y0.shape, dtype=self.y0.dtype, device=self.y0.device)
        solution[0] = self.y0
        j = 1
